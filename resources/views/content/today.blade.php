            ...
            <div class="panel-heading">
                <div class="col-md-6">                                     
                  <!-- <h6 class="panel-title txt-dark">Penerimaan PBB  : {{ Carbon\Carbon::now()->isoFormat("dddd, D MMMM Y") }} </h6> -->
				   <h6 class="panel-title txt-dark">Penerimaan PBB: {{ \Carbon\Carbon::now()->setTimezone('Asia/Jakarta')->isoFormat("dddd, D MMMM Y") }}</h6>
                                ...
                                <tbody>
                                    @php $no = 1; @endphp
                                        @foreach($result as $p)
                                            ...
                                            <td class="text-center">{{ $p->kd_propinsi }}.{{ $p->kd_dati2 }}.{{ $p->kd_kecamatan }}.{{ $p->kd_kelurahan }}.{{ $p->kd_blok }}.{{ $p->no_urut }}.{{ $p->kd_jns_op }}</td>
                                            <td>{{ $p->nm_wp_sppt }}</td>
